...
/// Matches [summary_normalizer_v2] English fallbacks — hide section when only placeholder.
const _kFallbackDecisions = 'No clinical decisions mentioned in the conversation';
const _kFallbackMedications =
    'No medications mentioned or changed in the conversation';
const _kFallbackActions = 'No follow-up actions mentioned in the conversation';
...
class _VisitDetailsScreenState extends State<VisitDetailsScreen> {
  ...
  bool _shouldShowList(List<String> items, String singlePlaceholder) {
    if (items.isEmpty) return false;
    if (items.length == 1 && items.first.trim() == singlePlaceholder) {
      return false;
    }
    return true;
  }
  ...
  Widget _buildStructuredSummary() {
    return Column(
      ...
      children: [
        ...
        if (_shouldShowList(_decisions, _kFallbackDecisions))
          _buildListSection(title: 'Clinical Decisions', items: _decisions),
        if (_shouldShowList(_medications, _kFallbackMedications))
          _buildListSection(title: 'Medications', items: _medications),
        if (_shouldShowList(_actions, _kFallbackActions))
